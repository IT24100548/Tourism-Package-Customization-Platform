...
<head>
    ...
    <style>
        ...
        .payment-container {
            background: white; border-radius: 15px; box-shadow: 0 0 20px rgba(0,0,0,0.1);
            padding: 2rem; margin-top: 2rem; margin-bottom: 2rem;
        }
        .page-header {
            border-bottom: 2px solid #e9ecef; margin-bottom: 1.5rem; padding-bottom: 1rem;
        }
        ...
        .search-input {
            padding-left: 2.5rem; border-radius: 10px;
        ...
        .table { border-radius: 10px; overflow: hidden; box-shadow: 0 0 15px rgba(0,0,0,0.05); }
        .table thead { background-color: #f8f9fa; }
        .table th {
            border-top: none; font-weight: 600; text-transform: uppercase;
            font-size: 0.85rem; color: #495057;
        }
        .table td { vertical-align: middle; }
        .badge { padding: 0.5rem 0.8rem; font-weight: 500; border-radius: 6px; }
        .badge.bg-success { background-color: #198754 !important; }
        .badge.bg-warning { background-color: #ffc107 !important; color: #000; }
        .badge.bg-danger { background-color: #dc3545 !important; }
        .btn-group .btn {
            padding: 0.4rem 0.6rem; margin: 0 0.1rem; border-radius: 6px;
        }
        .btn-info { background-color: #0dcaf0; border: none; color: white; }
        .btn-info:hover { background-color: #0bacce; color: white; }
        ...
        .payment-number {
            font-family: monospace; color: #0d6efd; font-weight: 500;
        }
...
<div class="container">
    <div class="payment-container">
        <div class="page-header d-flex justify-content-between align-items-center">
            <h2><i class="fas fa-list-ul me-2 text-primary"></i>Payment List</h2>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                <thead>
                <tr>
                    <th><i class="fas fa-hashtag me-2"></i>Payment No</th>
...
<script>
    document.getElementById('searchInput').addEventListener('keyup', function () {
        ...
        rows.forEach(row => {
            ...
            row.style.display = name.includes(searchText) || mobile.includes(searchText) ? '' : 'none';
        });
    });
    ...
    function updateStatus(paymentNumber, status) {
        if (confirm('Mark this payment as completed?')) {
            fetch('${pageContext.request.contextPath}/payment/' + paymentNumber, {
                ...
                body: new URLSearchParams({ paymentStatus: status })
            }).then(res => res.ok ? location.reload() : alert('Failed to update'));
        }
    }
